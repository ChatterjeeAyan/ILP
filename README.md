...
Link prediction is a vital task in graph machine learning, involving the anticipation of connections between entities within a network. In the realm of drug discovery, link prediction takes the form of forecasting interactions between drugs and target genes. Likewise, in recommendation systems, link prediction entails suggesting items to users. In the context of co-authorship networks, predicting new connections signifies the potential collaboration between authors, often shedding light on forthcoming interdisciplinary research endeavors. In temporal graphs, link prediction ranges from friendship recommendations to introducing new devices in wireless networks and dynamic routing. However, a prevailing challenge in link prediction lies in the reliance on topological neighborhood information for making predictions. Consequently, predictions for nodes with low degrees, and especially for newly introduced nodes with no neighborhood data, tend to be inaccurate and misleading. State-of-the-art models frequently fall short when tasked with predicting interactions between a novel drug and an unexplored disease target or suggesting a new product to a recently onboarded user, a situation often referred to as the "cold-start" setting. In temporal graphs, the link prediction models often misplace a newly introduced entity in the evolving network. This paper delves into the issue of observation bias, inequity of data availability for different entities in a network, and explores how contemporary models struggle when it comes to making inductive link predictions for low-degree and previously isolated unseen nodes. Additionally, we harness informative node attributes generated by unsupervised pre-training beyond the observed graph in enhancing the overall generalizability of link prediction models.
